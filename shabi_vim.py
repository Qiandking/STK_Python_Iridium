...
# ./waf --pyrun=/test_data/test_zhihu.py
'''
import ns.core
import ns.network
...
# 读取存储在Windows下的数据
win_path = '/test_data'
data_0 = pd.read_csv(win_path + '/Iridium_datasIridium0_0.csv')
Time_0 = data_0['Time']
X_0 = list(data_0['x'])
Y_0 = list(data_0['y'])
Z_0 = list(data_0['z'])

data_1 = pd.read_csv(win_path + '/Iridium_datasIridium0_1.csv')
...
time_list = []
Time_list = []
# 对于每个时隙，每个位置均认为卫星相对静止，并进行一次仿真
for i in tqdm(range(1, len(data_0))):  # len(data_0)
    ...
    pointToPoint.SetDeviceAttribute("DataRate", ns.core.StringValue("14.5Gbps"))
    ...
    csma.SetChannelAttribute("DataRate", ns.core.StringValue("14.5Gbps"))
    ...
    serverApps.Stop(ns.core.Seconds(0.01))
    ...
    clientApps.Stop(ns.core.Seconds(0.01))
    ...
    t = os.popen('tcpdump -nn -tt -r third-0-1.pcap')
    s = t.readline()
    time_list.append(s[:8])
    Time_list.append(Time_0[i])
    t.close()
data = pd.DataFrame([Time_list, time_list])
data = data.T
data.to_csv('0_to_1.csv', header=['Time', 'Delta_Time(ms)'], index=False)
